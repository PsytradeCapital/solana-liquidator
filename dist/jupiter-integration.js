...
export async function getJupiterQuote(inputMint, outputMint, amount, slippageBps = 50, onlyDirectRoutes = false) {
...
export async function findOptimalSwapRoute(inputMint, outputMint, inputAmount, slippageBps, maxPriceImpactPct) {
...
export async function buildJupiterSwapTransaction(quote, userPublicKey, slippageBps) {
    const swapRequest = {
        quoteResponse: quote,
...
export function estimateSwapPriceUsd(route) {
...
export function estimateSwapOutputUsd(route) {
    return Number(route.outAmount) / 1000000;
}
