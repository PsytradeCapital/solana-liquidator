...
function isOracleAddress(address) {
...
function calculateBuilderTipBps(gasPriceGwei) {
...
function applyTipShare(grossProfit, tipBps) {
    return grossProfit.mul(tipBps).div(100);
}
async function estimateSwapOutput(provider, tokenIn, tokenOut, fee, amountIn) {
    if (!CONFIG.UNISWAP_V3_QUOTER_ADDRESS)
...
function buildFlashLoanContractInterface(provider, signer) {
    return new Contract(CONFIG.FLASHLOAN_CONTRACT_ADDRESS, FLASH_LOAN_ABI, signer);
}
function serializeTransactionFromResponse(tx) {
    if (tx.raw)
        return tx.raw;
...
async function simulateTransaction(provider, txRequest) {
...
async function buildFlashLoanTransaction(provider, wallet, amount, minDebtOut, minProfit) {
    const contract = buildFlashLoanContractInterface(provider, wallet);
    const populated = await contract.populateTransaction.executeFlashLiquidation(CONFIG.AAVE_POOL_ADDRESS, CONFIG.WETH_ADDRESS, amount, CONFIG.COLLATERAL_ASSET, CONFIG.LIQUIDATION_TARGET, parseBytes(CONFIG.LIQUIDATION_DATA), CONFIG.UNISWAP_V3_ROUTER_ADDRESS, CONFIG.SWAP_FEE, minDebtOut, minProfit);
...
async function signTransaction(wallet, txRequest) {
    return await wallet.signTransaction(txRequest);
}
async function executeBundle(provider, flashbotsProvider, oracleTxRaw, liquidationTxRaw, targetBlockNumber) {
...
async function handleOracleUpdate(txHash, provider, flashbotsProvider, wallet) {
...
async function run() {
    ...
    startPendingTxWatcher(async (txHash) => {
        ...
        try {
            ...
            if (tx && tx.data) {
                const oracleUpdates = await oracleMonitor.scanEthereumMempoolForOracleUpdates(provider, txHash, tx.data);
    ...
    wsProvider.on('close', (code) => console.warn('WS closed', code));
