...
/**
 * Forked simulation infrastructure: sub-5ms transaction validation with
 * parallel workers and pre-calculated gas/compute unit cache.
 *
 * Architecture:
 * - Ethereum: Local Anvil fork from mainnet state snapshot
 * - Solana: Local validator clone with recent blockheight
 * - Workers: Parallel process pool for candidate simulation
 * - Cache: Pre-computed gas costs and compute units
 */
...
export class SimulationCache {
  ...
  get(signature: string): CachedSimulation | null {
...
export class EthereumForkedSimulator {
  ...
  private anvilProcess: childProcess.ChildProcess | null = null;
  private anvilPort = 8545;
  private cache: SimulationCache;

  constructor(mainnetRpcUrl: string, cache: SimulationCache) {
  ...
  async batchSimulate(
    ...
    return Promise.all(
      txRequests.map((tx) => this.simulateTransaction(tx))
    );
  }
}
...
export class SolanaForkedSimulator {
  ...
  private cloneHeight: number | null = null;
  private cache: SimulationCache;

  constructor(rpcUrl: string, cache: SimulationCache) {
   ...
   */
  async initializeClone(): Promise<void> {
  ...
  async batchSimulate(
    ...
    return Promise.all(
      txs.map((tx) => this.simulateTransaction(tx))
    );
  }
}
...
export class SimulationWorkerPool {
   ...
   */
  async submit(simulate: () => Promise<SimulationResult>): Promise<SimulationResult> {
    return new Promise((resolve, reject) => {
...
export class ForkedSimulationOrchestrator {
  private ethereumSimulator: EthereumForkedSimulator | null = null;
  private solanaSimulator: SolanaForkedSimulator | null = null;
  private cache: SimulationCache;
   ...
   */
  async simulateEthereumTransaction(txRequest: any): Promise<SimulationResult> {
    if (!this.ethereumSimulator) {
      throw new Error('Ethereum simulator not initialized');
    }

    return this.workerPool.submit(() =>
      this.ethereumSimulator!.simulateTransaction(txRequest)
    );
  }
   ...
   */
  async simulateSolanaTransaction(tx: Transaction): Promise<SimulationResult> {
    if (!this.solanaSimulator) {
      throw new Error('Solana simulator not initialized');
    }

    return this.workerPool.submit(() =>
      this.solanaSimulator!.simulateTransaction(tx)
    );
  }
   ...
   */
  async batchSimulateEthereum(txRequests: any[]): Promise<SimulationResult[]> {
    if (!this.ethereumSimulator) {
      throw new Error('Ethereum simulator not initialized');
    }

    const tasks = txRequests.map((tx) => () =>
      this.ethereumSimulator!.simulateTransaction(tx)
    );
   ...
   */
  async batchSimulateSolana(txs: Transaction[]): Promise<SimulationResult[]> {
    if (!this.solanaSimulator) {
      throw new Error('Solana simulator not initialized');
    }
